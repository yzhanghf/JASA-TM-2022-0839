...
recordtstat1 = recordtstat1 - diag(diag(recordtstat1));
...
for i = 1:size(recordtstat,1)
    for j = 1:i
        if valstd((i-1)*size(recordtstat,1)+j)==0
            text(C((i-1)*size(recordtstat,1)+j), R((i-1)*size(recordtstat,1)+j), {string(vals((i-1)*size(recordtstat,1)+j)),strcat("--")}, ...
...
colorbar;
title({'Pairwise dependency, test statistic', 'Complete U-statistic'},'interpreter','latex');
