...
for(ell in 5:8){
  ...
  for(i in 1:2){
    dat = readMat(
        sprintf("./result/result_earthquake_inner%d%d30.mat",i,MC_times)
      );
    CI_all = dat$CI.all[,,ell_index]
    mu_true_all = readMat(paste("./result/result_earthquake_inner_comp",i,MC_times_2,".mat",sep = ""))
    mu_true= mu_true_all$mu.hat.all.set[ell_index]
    CI_list = CI_all
    m = dim(CI_list)[1]
    pdf(file=paste('./plot/earthquake, inner comparison type ',ell,'_column_',i,'.pdf',sep = ""))
...
for(i in 5:8){
    dat = readMat(paste("./result/result_earthquake_new",i,"2010.mat",sep = ""))
    CI_all = dat$CI.all
    mu_true_all = readMat(paste("./result/result_earthquake_true",i,".mat",sep = ""))
    mu_true= mu_true_all$mu.true
    CI_list = CI_all
    pdf(file=paste('./plot/earthquake, within comparison',i,'.pdf',sep = ""))
