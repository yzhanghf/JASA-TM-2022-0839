...
for(i in 1:2){
  for(j in (i+1):3){
    dat = readMat(paste("./result/all_dist_bt_10_",i,"_",j,"_15.mat",sep = ""))
    CI_all = dat$CI.all
    mu_true= 0
    CI_list = CI_all
    pdf(file=paste('./plot/starlight, group comparison',i,j,'.pdf',sep = ""))
...
for(i in 1:2){
    dat = readMat(paste("./result/all_innerdistance5",i,"_15.mat",sep = ""))
    CI_all = dat$CI.all
    mu_true_all = readMat(paste("./result/cluster",i,"CIless_comp.mat",sep = ""))
    mu_true= mean(mu_true_all$mu.hat)
    CI_list = CI_all
    pdf(file=paste('./plot/starlight, within comparison',i,'.pdf',sep = ""))
    par(mar=c(5.1, 5, 5, 2.7), mgp=c(3, 1.2, 0), las=0)
    m = dim(CI_list)[1]
